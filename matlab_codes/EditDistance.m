...
n=length(string2);
v=zeros(m+1,n+1);
num_s = 0; % # of substitution
num_i = 0; % # of insertion
num_d = 0; % # of deletion
...
%         end
%         % insertion
%         if ( (v(i+1,j)+1) < (v(i,j+1)+1) && (v(i+1,j)+1) < (v(i,j)+temp))
%             num_i = num_i +1;
%             v(i+1,j+1) = v(i+1,j)+1;
%         % deletion
%         elseif ( (v(i,j+1)+1) < (v(i+1,j)+1) && (v(i,j+1)+1) < (v(i,j)+temp))
%             num_d = num_d +1;
%             v(i+1,j+1) = v(i,j+1)+1;
%         % substitution
%         elseif ( (v(i,j)+temp) < (v(i,j+1)+1) && ((v(i,j)+temp) < v(i+1,j)+1))
%             num_s = num_s +1;
%             v(i+1,j+1) = v(i,j)+temp;
%         end
    end
end
%V=num_s + num_i + num_d;
